...
package com.santiago.javassist.bytecode.stackmap;
...
import com.santiago.javassist.bytecode.StackMapTable;
...
public abstract class TypeData {
    ...
    protected static abstract class TypeName extends TypeData {
        protected ArrayList<TypeData> equivalences;
        ...
        protected TypeName() {
            equivalences = new ArrayList<TypeData>();
        ...
        public void merge(TypeData neighbor) {
            ...
            ArrayList<TypeData> list = equivalences;
            ArrayList<TypeData> list2 = neighbor2.equivalences;
            if (list == list2)
                return;
        ...
        private static void add(ArrayList<TypeData> list, TypeData td) {
            int n = list.size();
            for (int i = 0; i < n; i++)
                if (list.get(i) == td)
                    return;
        ...
        public void evalExpectedType(ClassPool cp) throws BadBytecode {
            ...
            ArrayList<TypeData> equiv = this.equivalences;
            int n = equiv.size();
            String name = evalExpectedType2(equiv, n);
            if (name == null) {
                name = this.expectedName;
                for (int i = 0; i < n; i++) {
                    TypeData td = equiv.get(i);
        ...
        private String evalExpectedType2(ArrayList<TypeData> equiv, int n) throws BadBytecode {
         ...
         */
        public String getExpected() throws BadBytecode {
            ArrayList<TypeData> equiv = equivalences;
            if (equiv.size() == 1)
                return getName();
            else {
