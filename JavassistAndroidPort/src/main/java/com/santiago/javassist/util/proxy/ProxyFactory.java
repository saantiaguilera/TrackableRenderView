...
package com.santiago.javassist.util.proxy;
...
import com.santiago.javassist.bytecode.*;
...
public class ProxyFactory {
    ...
    private static WeakHashMap<ClassLoader, HashMap<String, ProxyDetails>> proxyCache = new WeakHashMap<ClassLoader, HashMap<String, ProxyDetails>>();
    ...
    private void createClass2(ClassLoader cl) {
        ...
        // synchronized (proxyCache) {
            ...
            if (details != null) {
                WeakReference<Class<?>> reference = details.proxyClass;
    ...
    private static Comparator<Entry<String, ?>> sorter = new Comparator<Entry<String, ?>>() {
    ...
    private void makeSortedMethodList()
    {
        ...
        signatureMethods = new ArrayList<Entry<String, Method>>(allMethods.entrySet());
    ...
    private void computeSignature(MethodFilter filter) // throws CannotCompileException
    {
        ...
        {
            Entry<String, Method> e = signatureMethods.get(idx);
            Method m = e.getValue();
        ...
        throws CannotCompileException
    {
        ...
        Iterator<Entry<String, Method>> it = signatureMethods.iterator();
        int index = 0;
        while (it.hasNext()) {
    ...
    private static void getMethods(HashMap<String, Method> hash, Class<?> clazz, Set<Class<?>> visitedClasses) {
        ...
        for (int i = 0; i < methods.length; i++)
            if (!Modifier.isPrivate(methods[i].getModifiers())) {
                ...
                Method oldMethod = hash.put(key, methods[i]); 
